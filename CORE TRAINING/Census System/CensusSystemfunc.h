...
void censusSystem_searchCitizen(){
	...
	if(resultIndex==0){
		...
		if(choice=='n'){
			...
			char result[255] = ".\\CORE TRAINING\\Census System\\results";
...
void censusSystem_init(){
	system("cls");
	FILE *fCitizen = fopen(".\\CORE TRAINING\\Census System\\censusData.csv", "r");
	...
	if(!fCitizen){
		...
		do{
			loadFlag = 1;
			if(counter == 5){
				fCitizen = fopen(".\\CORE TRAINING\\Census System\\censusData.csv", "r");
...
void censusSystem_loadData(){
	FILE *fCitizen = fopen(".\\CORE TRAINING\\Census System\\censusData.csv", "w");
	for(int i=0;i<censusSystem_STATE;++i){
