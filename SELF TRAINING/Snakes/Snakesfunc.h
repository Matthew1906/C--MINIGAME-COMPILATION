...
void snakes_setup(struct snakes_body *snakes){
	FILE *fmap = fopen(".\\SELF TRAINING\\Snakes\\SnakeMap.txt", "r");
	int index = 0;
...
void snakes_startgame(){
	...
	FILE *fscore = fopen(".\\SELF TRAINING\\Snakes\\SnakeLeaderboard.txt", "a");
	fprintf(fscore, "%s#%d\n", name, snakes_Score);
	fclose(fscore);
...
void snakes_leaderboard(){
	system("cls");
	FILE *fscore = fopen(".\\SELF TRAINING\\Snakes\\SnakeLeaderboard.txt", "r");
	char names[255];
	int scores;
