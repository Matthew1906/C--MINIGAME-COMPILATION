...
void hangman_startgame(){
	...
	while(choice_flag==1){
		...
		srand ( time(NULL));
   		...
   		random=(rand() % 12 );
  		char str[60][255];
  		FILE *fp = fopen(".\\SELF TRAINING\\Hangman\\HangmanAnswer.txt", "r");
		for(int i = 0; i < 60; i++) {
  	       	fscanf(fp, "%[^\n]\n", str[i]);
  		}
  		fclose(fp);
	...
	if(strcmpi(choice,"yes")==0){
		...
		FILE *fscore = fopen(".\\SELF TRAINING\\Hangman\\HangmanLeaderboard.txt","a");
...
void hangman_leaderboard(){
	system("cls");
	FILE *fscore = fopen(".\\SELF TRAINING\\Hangman\\HangmanLeaderboard.txt", "r");
	char names[255];
	int win, loss;
